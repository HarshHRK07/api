...
# Customize logging format for better readability when using mitmdump
def log_entry(flow):
    """
    Custom logging format.
    """
    log_str = f"{flow.client_conn.address[0]}:{flow.client_conn.address[1]} - [{flow.request.timestamp.strftime('%Y-%m-%d %H:%M:%S')}] {flow.request.method} {flow.request.url} - {flow.response.status_code}"
    ctx.log.info(log_str)

# Register log_entry function to be executed on each request
def response(flow):
    """
